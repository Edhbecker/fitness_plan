  ...
  return (
    <div id="resumo" className="space-y-6">
      <section className="card overflow-hidden">
        ...
        <div className="flex flex-col gap-4 px-5 pb-5 md:flex-row md:items-end md:px-7"><span className="-mt-9 flex size-18 items-center justify-center rounded-2xl border-4 border-white bg-accent text-xl font-bold text-primary-dark">{initials}</span><div className="flex-1"><h1 className="text-2xl font-bold text-foreground">{student.name}</h1><p className="mt-1 text-xs text-muted">{calculateAge(student.birthDate)} anos · {sexLabels[student.sex]} · {Number(student.heightCm)} cm · {student.objective}</p></div><div className="flex flex-wrap gap-2"><Link href={`/alunos/${student.id}/avaliacao`} className="btn-secondary"><ClipboardPlus size={15} /> Nova avaliação</Link><Link href={`/alunos/${student.id}/periodizacao`} className="btn-primary"><Dumbbell size={15} /> Abrir treino</Link></div></div>
